...
export function AdminPageClient() {
  ...
  const getStatusBadge = (status: Order["status"]) => {
    switch (status) {
      case "pending":
        return <Badge className="bg-zinc-800 text-zinc-300 border-zinc-700 gap-1.5 font-medium">Pending</Badge>;
      case "preparing":
        return <Badge className="bg-zinc-800 text-white border-zinc-700 gap-1.5 font-bold">Preparing</Badge>;
      case "served":
        return <Badge className="bg-zinc-900 text-zinc-400 border-zinc-800 gap-1.5">Served</Badge>;
      case "cancelled":
        return <Badge className="bg-zinc-900 text-zinc-500 border-zinc-800 gap-1.5">Cancelled</Badge>;
      default:
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-8 animate-in fade-in duration-300">
        ...
        <div className="flex items-center gap-2 border-b border-zinc-800 pb-4">
          ...
          >
            ...
            {activeOrders > 0 && (
              <Badge className="bg-white text-zinc-950 font-black text-[10px] px-1.5 py-0">
                {activeOrders}
              </Badge>
            )}
          </button>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 flex items-center justify-between shadow-xl">
                ...
                <div className="p-3 rounded-xl bg-zinc-800 border border-zinc-700">
                  <IndianRupee className="h-6 w-6 text-zinc-300" />
                </div>
              </div>
            </div>
                          ...
                          <>
                            <div className="flex flex-wrap items-center gap-2 mt-2 mb-3">
                              {order.payment_method === "upi" ? (
                                <Badge className="bg-zinc-800 text-white border-zinc-700 gap-1 text-[10px]">
                                  <QrCode className="h-3 w-3 text-zinc-300" />
                                  <span>UPI PAID</span>
                                </Badge>
                              ) : (
                                <Badge className="bg-zinc-800 text-zinc-300 border-zinc-700 gap-1 text-[10px]">
                              ...
                              {order.table_token && (
                                <Badge className="bg-zinc-800 text-white border-zinc-700 gap-1 text-[10px] font-mono font-bold tracking-wider">
                                  <Key className="h-3 w-3 text-zinc-300" />
                                  <span>TOKEN: {order.table_token}</span>
                                </Badge>
                              )}

                              {lockTime ? (
                                <Badge className="bg-zinc-950 text-zinc-300 border-zinc-800 gap-1 text-[10px] font-mono animate-pulse">
                            ...
                            {order.notes && (
                              <div className="mt-2 p-2.5 rounded-lg bg-zinc-950 border border-zinc-800 text-xs text-zinc-300 flex items-start gap-2">
                                <Sparkles className="h-3.5 w-3.5 text-zinc-400 shrink-0 mt-0.5" />
                                <p className="leading-relaxed">
                                  <strong className="text-white font-medium">Note:</strong> {order.notes}
                                </p>
                              </div>
                            )}
                            ...
                            {lockTime && (
                              <button
                                ...
                                className="mt-3 w-full py-1.5 px-3 rounded-xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white transition-colors text-xs font-semibold flex items-center justify-center gap-1.5"
                              >
                                <Unlock className="h-3.5 w-3.5 text-zinc-300" />
                      ...
                      <div className="flex items-center gap-1.5 flex-1">
                        {order.status === "pending" && (
                          <Button
                            ...
                            className="flex-1 gap-1 text-xs bg-white hover:bg-zinc-200 text-zinc-950 font-bold"
                          >
                        ...
                        {order.status === "served" && (
                          <span className="text-xs text-zinc-400 font-semibold flex items-center gap-1">
                            <CheckCircle2 className="h-4 w-4 text-zinc-400" /> Served to Table
                          </span>
                        )}
                      </div>
